...
class GlobalConfig:
    def __init__(self, data):
        ...
        self.theme = Theme(data.get("theme", {
            ...
            "success": "bold green",
            "question": "bold white",
            "flag": "bold green reverse",
            "prompt": "bold yellow"
        }))
        
        self.all_correct = True
        self.questions_data = data.get("questions", [])
        self.length = len(self.questions_data)
...
if __name__ == "__main__":
    ...
    if config.header_text:
        ...
        console.print(Panel(f"[{config.header_style}]{config.header_text}[/{config.header_style}]", 
                            border_style="bright_black"), justify="center")
        console.print("\n")

    # Main Loop
    for i, q_data in enumerate(config.questions_data, 1):
        q = Question(i, q_data, config)
        
        console.print(Rule(f"[info]Question {q.number} / {config.length}[/info]"))
